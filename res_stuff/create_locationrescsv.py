...
location_res = original_df[['name', 'address', 'cuisine', 'lat', 'lon', 'opening_hours', 'image_url', 'rating']].copy()
...
cuisine_list=[]
num_list=[]
for i,cuisines in enumerate(location_res['cuisine']):
    try:
...
location_res['cuisine'] = cuisine_list

# AFTER CONVERTING ALL "" TO '
location_res['opening_hours'] = location_res['opening_hours'].apply(lambda x: ast.literal_eval(x.replace("true", "True").replace("false", "False")))

# location_res.to_csv(r"path\to\output\csv\file")
location_res.to_csv(r"eatsee_db\res_stuff\res_data\location_res.csv")
